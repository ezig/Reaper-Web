...
function gen_histogram(csvdata, div_id) {
  ...
  for (var i = 0; i < csvdata.length; i++) {
    if (csvdata[i].c2 in cnt_data) {
      ...
      if (cnt_data[csvdata[i].c2] > ymax) ymax = cnt_data[csvdata[i].c2];
    } else {
  ...
  var tip = d3.tip().attr('class', 'd3-tip').direction('e').offset([0, 20]).html(function (d) {
    return '<table id="tiptable">' + "<tr><td>Year: " + d.meta + "</td></tr><tr><td> Count: " + d.numfill + "</td></tr></table>";
  });

  // put the graph in the "mpg" div
  var svg = d3.select(div_id).append("svg").attr("width", width + margin.left + margin.right).attr("height", height + margin.top + margin.bottom).append("g").attr("transform", "translate(" + margin.left + "," + margin.top + ")");

  svg.call(tip);

  // set up the bars
  var bar = svg.selectAll(".bar").data(histdata).enter().append("g").attr("class", "bar").attr("transform", function (d, i) {
    return "translate(" + (x(d.meta * binsize) - binwidth / 2) + "," + y(d.numfill) + ")";
...
function build_pubviz(csvdata, div_id) {
  ...
  // This scale is for determining the widths of the histogram bars
  // Must start at 0 or else x(binsize a.k.a dx) will be negative
  var x = d3.scaleLinear().domain([0, xmax - xmin]).range([0, width]);

  // Scale for the placement of the bars
  var x2 = d3.scaleLinear().domain([xmin - 1, xmax + 1]).range([0, width]);

  var y = d3.scaleLinear().domain([-1, 56]).range([height, 0]);
  ...
  // set up the bars
  var bar = svg.selectAll(".circle").data(xydata).enter().append("g").attr("class", "circle").on('mouseover', tip.show).on('mouseout', tip.hide);

  circle_radius = 0.98 * (height / (55 + 2)) / 2;

  // add rectangles of correct size at correct location
  bar.append("circle").attr("cx", function (d) {
    return x2(d[0]);
  }).attr("cy", function (d) {
    return y(d[1]);
  }).attr("r", function (d) {
