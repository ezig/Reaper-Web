...
function gen_input_panel_btn(container_id, upload_btn_id, target_div_id) {
  ...
  $("#add_sub_input_example_btn" + panel_id).click(function () {
    ...
    gen_synthesize_btn("synthesize-btn-container" + panel_id, "synthesize-btn" + panel_id, sub_panel_id_list, "otable_" + panel_id, "constraint-panel" + panel_id, "display-panel" + panel_id);
  });
...
function gen_synthesize_btn(container_id, btn_id, target_input_div_id_list, target_output_div_id, target_constraint_div_id, target_display_panel_id) {

  synthesize_btn = "<div class=\"buttons\" style=\"padding-left:10px;padding-right:10px;\">\
            					<button id=\"" + btn_id + "\" target_input_div_id=\"" + serialize_id_list(target_input_div_id_list) + "\" target_output_div_id=\"" + target_output_div_id + "\" target_display_panel_id=\"" + target_display_panel_id + "\" class=\"btn btn-primary btn-block\">Synthesize</button>\
          					</div>";
  ...
  $("#" + btn_id).on("click", function () {
    ...
    if (aggr_function_string == "") aggr_function_string = '"max", "min", "count"';
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        if (vis_type == "2dhistogram") {
          var map = {};
          for (var i = 0; i < csvdata.length; i++) {
            if (csvdata[i].career_length == "" || csvdata[i].paper_year == "" || isNaN(csvdata[i].career_length) || isNaN(csvdata[i].paper_year)) continue;
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        for (var i = data.length - 1; i >= 0; i--) {
          temp_tab_content = '<div id="query_inner_' + id + '_tabs_' + (data.length - i) + '">' + "<pre style=\"height:100%; overflow:auto; margin: 0 0 5px;\"><span id='synthesized_query_" + id + "_" + (data.length - i) + "' class=\"inner-pre\" style=\"font-size: 12px\">" + htmlForTextWithEmbeddedNewlines(data[i]) + '</span></pre></div>';
          query_content_list.push(temp_tab_content);
        }

        concat_links = "";
        for (var i = data.length - 1; i >= 0; i--) {
          temp_link = "<li><input type='radio'" + "id='link_to_query_inner_" + id + "_tabs_" + (data.length - i) + "' name='link_to_query_inner_" + id + "' value='1' " + (i == data.length - 1 ? "checked=''" : "") + ">" + "<label for='link_to_query_inner_" + id + "_tabs_" + (data.length - i) + "'> Query#" + (data.length - i) + "</label></li>";
          concat_links += temp_link;
        }

        // the default query content
        query_content = '<div class="query_output_container" id="query_inner' + id + '">' + query_content_list[query_content_list.length - 1] + "</div>";
...
function serialize_id_list(id_list) {
  return id_list.join("#%#@");
}
function deserialize_id_list(id_list_string) {
  return id_list_string.split("#%#@");
}
...
function gen_adjustable_table_div(container_id, table_div_id) {
  ...
  html_frame = "<div id=\"" + table_div_id + "\" class=\"tbl\">\
      <table id=\"" + table_content_id + "\" class=\"hover cell-border compact dataTable no-footer\">\
        ...
        <tbody>" + default_content + "</tbody>\
      </table>\
      <div class=\"buttons\">\
        <input type=\"text\" size=\"10\" class=\"table_name\" id=\"" + table_name_id + "\" value=\"tablename\">\
        <button id=\"" + insert_row_btn_id + "\" class=\"btn btn-super-sm btn-default\">\
              <span class=\"glyphicon glyphicon-plus\"></span> R</button>\
        <button id=\"" + delete_row_btn_id + "\" class=\"btn btn-super-sm btn-default\">\
              <span class=\"glyphicon glyphicon-minus\"></span> R</button>\
        <button id=\"" + insert_col_btn_id + "\" class=\"btn btn-super-sm btn-default\">\
              <span class=\"glyphicon glyphicon-plus\"></span> C</button>\
        <button id=\"" + delete_col_btn_id + "\" class=\"btn btn-super-sm btn-default\">\
...
function build_visualization(csvdata, target_div_id, vis_type) {
  // clear the diagram
  $(target_div_id).html("");
  if (vis_type == 1) gen_histogram(csvdata, target_div_id);else if (vis_type == 2) build_pubviz(csvdata, target_div_id);
}